...
//get factory flow by Factory
export const getAllFactoryFlow = async (req, res) => {
  try {
    const rows = await sql`SELECT * FROM "Factory_flow" `;
...
export const getAllFactoryFlowById = async (req, res) => {
  const { id } = req.params;
  try {
    const [row] = await sql`SELECT * FROM "Factory_flow" WHERE id = ${id}`;
 ...
 */
export const deleteFactoryFlow = async (req, res) => {
  const { id } = req.params;
  try {
    const [row] =
      await sql`DELETE FROM "Factory_flow" WHERE id = ${id} RETURNING *`;
    if (!row) return res.status(404).json({ error: "Not found" });
