...
//get all receptions
export const getAllProcessedGrades = async (req, res) => {
  try {
    const rows =
...
export const getAllProcessedGradesById = async (req, res) => {
  const { id } = req.params;
  try {
    const [row] =
      await sql`SELECT * FROM "FieldSupply" WHERE productnature_id = 2 AND id = ${id}`;
