  ...
  try {
    ...
    const daily = dailyRows.map((r) => {
      const actual =
        dailyReception.find((x) => x.cropTypeId === r.tbl_CropTypeId)
          ?.actualReception ?? 0;
      const estimateKg = Math.round(Number(r.DailyEst) * 1000);
      return {
        cropTypeId: r.tbl_CropTypeId,
        crop_type: r.crop_type,
        dailyEstimate: estimateKg,
        actualReception: actual,
        variance: actual - estimateKg,
      };
    });
    ...
    const weekly = weeklyRows.map((r) => {
      const actual =
        weeklyReception.find((x) => x.cropTypeId === r.tbl_CropTypeId)
          ?.actualReception ?? 0;
      const estimateKg = Math.round(Number(r.WkToDateEst) * 1000);
      return {
        cropTypeId: r.tbl_CropTypeId,
        crop_type: r.crop_type,
        weeklyEstimate: estimateKg,
        actualReception: actual,
        variance: actual - estimateKg,
      };
    });
    ...
    monthlyRows.forEach((r) => {
      const actual =
        monthlyReception.find((x) => x.cropTypeId === r.tbl_CropTypeId)
          ?.actualReception ?? 0;
      const estimateKg = Math.round(Number(r.MonthToDateEst) * 1000);
      monthly[monthKey][r.crop_type] = {
        estimate: estimateKg,
        actual,
        variance: actual - estimateKg,
    ...
    yearlyRows.forEach((r) => {
      const actual =
        yearlyReception.find((x) => x.cropTypeId === r.tbl_CropTypeId)
          ?.actualReception ?? 0;
      const estimateKg = Math.round(Number(r.YearToDateEst) * 1000);
      yearly[r.crop_type] = {
        estimate: estimateKg,
        actual,
        variance: actual - estimateKg,
